...
function VISettingsPage() {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      let storedEmail = await AsyncStorage.getItem('viEmail');
      if (storedEmail) {
        storedEmail = storedEmail.replace(/['"]+/g, '');
        setEmail(storedEmail);
      }
      let storedBuilding = await AsyncStorage.getItem('viBuilding');
      if (storedBuilding) {
    ...
    <View>
      <View style={styles.infoContainer}>
        ...
        <View style={styles.infoRightContainer}>
          <Text style={styles.subTitle}>視障人士</Text>
        </View>
      </View>
